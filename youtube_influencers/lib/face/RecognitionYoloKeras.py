...
root = "lib/face/YoloKerasFaceDetection/"
#root = "YoloKerasFaceDetection/"


class RecognitionYoloKeras:
    ...
    def get_predicted_data(self):
        ...
        results = self.interpret_output_yolov2(out2, img.shape[1], img.shape[0])
        # Age and Gender Detection
        data = self.show_results(img_cv, results, img.shape[1], img.shape[0], model_age, model_gender)
        if len(data) != 0:
            return data[0]
        else:
            return None

    def interpret_output_yolov1(self, output, img_width, img_height):
        classes = ["face"]
        w_img = img_width
        h_img = img_height
        threshold = 0.2
    ...
    def interpret_output_yolov2(self, output, img_width, img_height):
        ...
        for i in range(len(boxes)):
            ...
            result.append([predicted_class, (boxes[i].xmax + boxes[i].xmin) * img_width / 2,
                           (boxes[i].ymax + boxes[i].ymin) * img_height / 2,
                           (boxes[i].xmax - boxes[i].xmin) * img_width,
    ...
    # crop
    def crop(self, x, y, w, h, margin, img_width, img_height):
        xmin = int(x - w * margin)
        xmax = int(x + w * margin)
        ymin = int(y - h * margin)
        ymax = int(y + h * margin)
    ...
    # display result
    def show_results(self, img, results, img_width, img_height, model_age, model_gender):
        output = []
        img_cp = img.copy()
        for i in range(len(results)):
            ...
            xmin, xmax, ymin, ymax = self.crop(x, y, w, h, 1.0, img_width, img_height)
            ...
            xmin2, xmax2, ymin2, ymax2 = self.crop(x, y, w, h, 1.1, img_width, img_height)
            ...
            lines_age = open(root + 'words/agegender_age_words.txt').readlines()
            lines_gender_dirty = open(root + 'words/agegender_gender_words.txt').readlines()
            lines_gender = list(map(lambda x: x.rstrip("\n"), lines_gender_dirty))
            lines_fer2013 = open(root + 'words/emotion_words.txt').readlines()
            ...
            if (model_gender != None):
                ...
                #gender_result = "Gender : %.2f" % prob_gender_keras + " " + lines_gender[cls_gender_keras]
                gender_result = lines_gender[cls_gender_keras]
                #cv2.putText(target_image, gender_text,                          (xmin2, ymax2 + offset), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0, 0, 250));
                offset = offset + 16
...
def main():
    pred = RecognitionYoloKeras(
        "https://i1.adis.ws/i/canon/get-inspired-home-studio-portraits-8_818a366a3c20449ba4eeaad3f174b490?$media-collection-full-dt-jpg")
    data = pred.get_predicted_data()
    print(data)

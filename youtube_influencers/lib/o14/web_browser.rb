...
module O14
  module WebBrowser
    def self.get_driver
      ...
      @@driver ||= begin
        ...
        when 'firefox'
          ...
          if @use_proxy
            puts "set proxy = #{@proxy_host}:#{@proxy_port}"
            proxy = Selenium::WebDriver::Proxy.new(type: :manual)
            proxy.http = "#{@proxy_host}:#{@proxy_port}"
            proxy.ssl = "#{@proxy_host}:#{@proxy_port}"
        ...
        when 'chrome'
          ...
          if @@profile_dir
            O14::ProjectLogger.get_logger.debug "--user-data-dir=#{get_profile_dir_path}"
            options.add_argument("--user-data-dir=#{get_profile_dir_path}")
          end
          if @@proxy
            O14::ProjectLogger.get_logger.debug "--proxy-server=#{@@proxy[:host]}:#{@@proxy[:port]}"
    ...
    def self.set_profile_dir profile_dir
